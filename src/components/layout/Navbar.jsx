...
import { Search, Plus, Bell, Command, UserPlus, DollarSign } from 'lucide-react';

export const Navbar = ({ onOpenNewClientModal, onOpenNewDealModal }) => {
  const { activeTab, setIsSearchOpen, tasks } = useCRM();
    ...
    <header className="h-16 border-b border-slate-800 bg-slate-900/80 backdrop-blur-md px-6 flex items-center justify-between sticky top-0 z-20">
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          className="flex items-center gap-3 bg-slate-800/80 hover:bg-slate-800 text-slate-400 hover:text-slate-200 border border-slate-700/60 px-3.5 py-1.5 rounded-xl text-xs font-medium transition-all shadow-sm group w-64 justify-between"
        >
          <div className="flex items-center gap-2">
            <Search className="w-3.5 h-3.5 text-slate-400 group-hover:text-indigo-400" />
            <span>Buscar cliente, trato, tarea...</span>
          </div>
